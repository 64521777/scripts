...
do
    ...
    file_encoding=`file $file | awk -F : '{print $2}' | awk -F , '{print (NF > 1 ? $2 : $1)}' | awk '{print $1}'`
    #GBK
    if [ $file_encoding == "UTF-8" ];
    then
        original_encoding="utf-8"
    #ASCII or UTF-8
    elif [ $file_encoding == "ASCII" -o $file_encoding == "ISO-8859" ];
    then
    ...
    echo "convert $file"
    # iconv -f gbk -t gbk $file > $file"_tmp"
    #判断是否转换成功
    #if [ $? != 0 ];
    #then
